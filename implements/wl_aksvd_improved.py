...
for run in range(N_RUNS):
    ...
    # Vocabulary size after the adaptive (energy) cut
...
# ── Summary ─────────────────────────────────────────────────────────────
summary_lines = [
    f"WL+KSVD (improved) results over {N_RUNS} runs",
...
for model_name in MODEL_NAMES:
    ...
    # ── Per-run detail ──
    header = (f"  {'run':>4} {'seed':>6} {'features':>10} "
    ...
    for i in range(N_RUNS):
        ...
        row = (
            f"  {i+1:>4} {run_seeds[i]:>6} {features:>10} "
...
results_path = os.path.join(
    RESULTS_DIR,
    f"wl_aksvd_improved_results_nci_{graphDataLoader.dataset_id}_{n_dimensions}_{RUN_TIMESTAMP}.txt",
)
